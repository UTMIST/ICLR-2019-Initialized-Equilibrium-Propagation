    ...
    for epoch in range(hyperparams["epochs"]):
       # shuffle train set
        ...
        for minibatch in range(int(np.ceil(train_size/hyperparams["minibatch"]))):
            x_sample = train[0][:, shuffled_indices[minibatch * hyperparams["minibatch"] : (minibatch + 1) * hyperparams["minibatch"]]]
            y_sample = train[1][:, shuffled_indices[minibatch * hyperparams["minibatch"]: (minibatch + 1) * hyperparams["minibatch"]]]
            ...
            model.update_weights(beta, hyperparams["eta"], s_pos, s_neg, x_sample)

if __name__ == "__main__":
    hyperparams = {
        "epsilon": 0.5,        # step size
        "beta": 0.5,              # clamping factor
        "eta": 0.1,             # learning rate, or .05?
